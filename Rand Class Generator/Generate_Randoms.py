...
def main():
    ...
    spectra_source_dir = sys.argv[1]
    filters_source_dir = sys.argv[2]
    outdir = sys.argv[3]
    n = int(sys.argv[4])
    start_spectrum_number = int(sys.argv[5])  # Adjust for 1-based indexing
    start_filter_number = int(sys.argv[6])  # Adjust for 1-based indexing
    ...
    for i in range(m):
        for j in range(n):
            ...
            for k, line in enumerate(filter_lines):
                    ...
                    line[0] = 1.0
                    filters_array[i, j, k] = line

    # Apply filters and save filtered spectra
    for i, classname in enumerate(classnames):
        for j in range(n):
            filtered_spectrum = np.multiply(spectra_array[i, j], filters_array[i, j])
