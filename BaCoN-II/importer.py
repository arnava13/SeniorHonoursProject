...
def evaluate_accuracy(model, test_dataset, out_path, names=None, FLAGS=None):
    acc_total=0
    y_true_tot=[]
    y_pred_tot=[]
...
def evaluate_accuracy_bayes(model, test_dataset, out_path, num_monte_carlo=50, th_prob=0.5, names=None, FLAGS=None):
    acc_total=0
    acc_total_no_uncl=0
    y_true_tot=[]
    y_pred_tot=[]
...
def main():
    ...
    if FLAGS.bayesian:
        _ = evaluate_accuracy_bayes(model_loaded, test_dataset, out_path,  
                                    num_monte_carlo = args.n_monte_carlo_samples, th_prob=args.th_prob, 
                                    names=names, FLAGS=FLAGS)
    else:
         _ = evaluate_accuracy(model_loaded, test_dataset, out_path,  names=names, FLAGS=FLAGS)
