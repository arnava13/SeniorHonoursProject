...
def my_train(model, loss, epochs,
    ...
    if TPU:
        with strategy.scope():
            val_steps_per_epoch = (val_dataset.n_classes_out * len(val_dataset.list_IDs)) // (val_dataset.batch_size * strategy.num_replicas_in_sync)
            train_steps_per_epoch = (train_dataset.n_classes_out * len(train_dataset.list_IDs)) // (train_dataset.batch_size * strategy.num_replicas_in_sync)
