...
def my_train(model, loss, epochs,
    ...
    if TPU:
        with strategy.scope():
            val_steps_per_epoch = val_dataset.n_indexes // (val_dataset.batch_size*strategy.num_replicas_in_sync)
            train_steps_per_epoch = train_dataset.n_indexes // (train_dataset.batch_size*strategy.num_replicas_in_sync)
