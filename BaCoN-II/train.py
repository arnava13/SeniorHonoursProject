...
def my_train(model, loss, epochs,
    ...
    if TPU:
        with strategy.scope():
            val_steps_per_epoch = len(val_dataset) // (val_dataset.batch_size*strategy.num_replicas_in_sync)
            train_steps_per_epoch = len(train_dataset) // (train_dataset.batch_size*strategy.num_replicas_in_sync)
